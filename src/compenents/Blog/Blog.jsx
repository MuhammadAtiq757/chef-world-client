...
const Blog = () => {
    const pdfGenerate = () => {
        ...
        doc.text(40, 40, `
        first qus ans: In software development, the terms "controlled" and
        "uncontrolled" components typically refer to different ways of 
        managing user input in a user interface. depends on the state of another.

        second quse ans:js web application framework that provides a robust set
        of features for web and mobile applications. In other words, NodeJS is
        the package, which provides the JavaScript run-time environment, whereas Express is
        a framework that sits on top of NodeJS and helps us to handle requests and responses.

        third ques ans: React PropTypes validators
        PropTypes.any : The prop can be of any data type.
        PropTypes.bool : The prop should be a Boolean.
        PropTypes.number : The prop should be a number.

        last ques ans: Custom React JS hooks offer reusability as when a 
        custom hook is created, and again while rendering the whole code.`)
        doc.save('atiq.pdf')
    }
